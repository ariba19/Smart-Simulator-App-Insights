      ...
      .subscribe((res) => {
        ...
        if (this.r === this.inputdate) {
          res['products'].forEach((x) => {
            if (x.date === this.todaysdate && x.nowtime!==undefined) {
              console.log(x.nowtime+" helloooooooooooooooooo");
          ...
          this.apimap = new Map();
          res['products'].forEach((x) => {
            if (x.date === this.todaysdate && x.nowtime!==undefined && Math.abs((x.nowtime.substring(0, 2)) - this.datee) <= 1 && x.api.indexOf('mioperations') === -1 && x.api.indexOf('fetchuserroles') === -1 && x.api.indexOf('regiondetails') === -1) {
          ...
          res['products'].forEach((x) => {
            ...
            if (x.date === this.todaysdate && x.nowtime!==undefined && Math.abs((x.nowtime.substring(0, 2)) - this.datee) <= 1) {
